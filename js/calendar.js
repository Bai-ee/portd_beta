...
const generateMeetingsHtml = (meetings = []) => {
  ...
  try {
    ...
    reversedMeetings.map((meeting, index) => {
        ...
        meeting.eventHtmlLink || ""
                          ...
                          <div class="display-flex flex-col ml-3">
                            <div id="" class="cont_vert">
                              ...
                              <div style="height:20px" class="display-flex flex-row justify-start align-center mt-0">
      ...
      if (meeting.link?.length > 0) {
        meetingsHtml += `
              ...
              <div id="${"meeting_note_edit_"}" class="meeting_edit_icon meeting_card_icon" dataLink="${
          meeting.eventHtmlLink || ""
        }">
            ...
            <p style="font-size: 8px" class="mt-0">event</p>
            </div>
          ...
          <p style="font-size: 8px" class="mt-0">note</p>
          </div>
                ...
                <p style="font-size: 8px" class="mt-0">meet</p>
                </div>
        ...
        meetingsHtml += `
                <div class="display-flex flex-row justify-center align-center mr-3">
                <div id="${"meeting_note_edit_"}" class=" meeting_card_icon" dataLink="${
          meeting.eventHtmlLink || ""
        }">
    ...
    meetingsHtml =
      meetingsHtml +
      `<button id="video_call_new_meetings_card" style="gap:5px" class="new_meetings_card display-flex" style="width:97.5%">
      <span style="pointer-events:none;" class="material-symbols-outlined calendar_add_on">
      calendar_add_on
      </span>
      <p class="mt-1 calendar_new_meetings_card" id="newMeetingCal" style="">new meeting</p>
    </button>`;
...
const injectMeetingsHtml = (meetingsHtml) => {
  ...
  if (meetingsHtml) {
    ...
    const viewMeeting = document.querySelectorAll("#meeting_note_edit_");
    // console.log("View Meetings: ", viewMeeting);
    viewMeeting.forEach((meetingCard) => {
      meetingCard.onclick = async () => {
        // console.log("Clicked: ", meetingCard.getAttribute("dataLink"))
        chrome.tabs.create({
          url: meetingCard.getAttribute("dataLink"),
        });
      };
    });
