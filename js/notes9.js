  ...
  if (children?.length)
    children?.forEach((child) => {
      if (child.tagName === "DIV") {
        ...
        child.src = "../assets/calendar_white.png";
      } else if (child.tagName === "P") {
        child.innerHTML = "New Meeting";
      }
    });
};
...
const showMeetingsUI = () => {
  ...
  document.getElementById("add_notes").style.opacity = 1;
  document.getElementById("add_notes").style.pointerEvents = "all";
  ...
  if (children?.length)
    children?.forEach((child) => {
      if (child.tagName === "DIV") {
        ...
        child.innerHTML = "Back to Meetings";
      }
    });
};
...
const showCreateEventsUI = () => {
  ...
  document.getElementById("add_notes").style.opacity = 0;
...
const dealBrowserEvents = async () => {
  ...
  try {
    ...
    const refershButton = document.getElementById("refresh-data");

    refershButton.addEventListener("click", async () => {
      refershButton.classList.add("refresh-icon-rotate");
      ...
      setTimeout(() => {
        refershButton.classList.remove("refresh-icon-rotate");
        // quillDefault.focus();
        const noteTitle = document.getElementById("note-typing-title-0");
        noteTitle.focus();
      }, 1000);
...
// Thois function is to extract the note data from the active selection option
  ...
  try {
    ...
    const toggleCollapsible = (event) => {
      ...
      if (isHalfOpen) {
        ...
        else document.getElementById("previous-notes").style.display = "none";
